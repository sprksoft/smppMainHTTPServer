...
const timeTitlesENG = [
    window.translations?.time_first_break || "First Break🍎",
    window.translations?.time_lunch || "Lunch🥪",
    window.translations?.time_school_out || "School's Out!🏫",
    window.translations?.time_night_checkin || "Night Check-in💤"
]
const timeTitlesMobileENG = [
    window.translations?.time_first_break_mobile || "Break🍎",
    window.translations?.time_lunch_mobile || "Lunch🥪",
    window.translations?.time_school_out_mobile || "Done! 🏫",
    window.translations?.time_night_checkin_mobile || "Night💤"
]
const prodPreviewDescriptionsENG = [
    window.translations?.product_description_weather || "See if it's <a href=\"https://open.spotify.com/track/2QjOHCTQ1Jl3zawyYOpxh6?si=62dad11bc7384e79\" target=\"_blank\" class=\"accent-text\"> sweater weather </a> outside",
    window.translations?.product_description_planner || "Check which <span class=\"accent-text\">lessons</span> you have in the afternoon",
    window.translations?.product_description_delijn || "Find out when <span class=\"accent-text\">your bus</span> will arrive",
    window.translations?.product_description_homework || "Quickly check if you made your <span class=\"accent-text\">homework</span>"
]
...
const prodPreviewTitles = [
    window.translations?.product_weather || "Weather 🌧️",
    window.translations?.product_planner || "Planner 📆",
    window.translations?.product_delijn || "Delijn 🚌",
    window.translations?.product_planner || "Planner 📆"
]
...
function updateFunPreview(shouldAnimate, extraFun) {
    ...
    if (extraFun) {
        ...
        document.getElementById("volume-text").innerHTML = window.translations?.volume_turn_on || "Please turn on volume for the full experience"
        ...
        document.getElementById("volume-icon").addEventListener("click", function () {
            ...
            document.getElementById("volume-text").innerHTML = window.translations?.volume_continue || "Press <span id=\"fun-continue-button\" class=\"accent-text\">here</span> to continue"
            document.getElementById("volume-icon").classList.remove("volume-icon-hover")
            document.getElementById("fun-continue-button").addEventListener("click", function () {
...
const words = window.translations ? window.translations.typewriter : ['Games', 'Widgets', 'Themes','Global Chat','Fun'];
